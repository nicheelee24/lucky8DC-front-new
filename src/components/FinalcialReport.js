...
export const FinancialReport = () => {
  ...
  return (
    <div className="max-w-[1200px] m-auto w-full mt-[30px] px-4 md:px-0">
      <div className="lg:flex md:flex sm:flex justify-between items-center mb-4 ">
        <h1 className="mb-4 lg:mb-0 md:mb-0 sm:mb-0 text-lg font-bold text-[var(--secondaryColor)]">
          Transactions History
        </h1>
        <div className="flex flex-wrap sm:flex-nowrap items-center justify-center gap-2 sm:gap-3">
          <input
            type="date"
            className="border p-1 py-2 text-sm rounded w-full sm:w-auto"
          ...
          />
          <button
            className="bg-[var(--logoutBg)] text-black font-bold py-2 px-4 rounded w-full sm:w-auto"
            ...
            <table className="w-full min-w-max text-sm text-left">
              <thead className="text-xs text-[var(--textPrimaryColor)] uppercase">
                <tr>
                  <th
                    scope="col"
                    className="whitespace-nowrap py-3 px-6 border-b-2 border-white bg-gray-1200 text-left text-xs font-bold uppercase tracking-wider text-white"
                  >
                    Date
                  </th>
                  <th
              ...
              <tbody>
                {currentItems.length > 0 ? (
                  <>
                    {currentItems.map((tableData, index) => {
                      return (
                        <tr className="text-white border-b border-[#bbb]">
                          ...
                          <td className="py-4 px-6">{tableData.CATEGORY}</td>
                          <td className="text-right py-4 px-6">
                            {tableData.ALLCOMPUTERS.length === 2 ? (
                              <>
                                <div className="text-white">
                                  {tableData.ALLCOMPUTERS[0]}
                                </div>
                                <div className="text-white">
                                  {tableData.ALLCOMPUTERS[1]}
                                </div>
                              </>
                            ) : (
                              <div className="text-white">
                          ...
                          <td className="py-4 px-6">{tableData.TOTALAMOUNT}</td>
                        </tr>
                      );
                    })}
          ...
          <div className="pagination flex justify-end mt-4 items-center">
            ...
            {pageNumbers.map((number, index) => {
              ...
              if (
                ...
                return (
                  <span key={number} className="px-3 py-1 mx-1">
                    ...
                  </span>
                );
              } else {
