...
export const MyBet = () => {
  ...
  const currentBets = Array.isArray(mybets)
    ? mybets.slice(indexOfFirstBet, indexOfLastBet)
    : [];
  ...
  return (
    <div className="max-w-[1200px] m-auto w-full mt-[30px] px-4 md:px-0">
      <div className="lg:flex md:flex sm:flex justify-between items-center mb-4">
        <h1 className="text-xl mb-4 lg:text-lg md:text-lg sm:text-lg font-semibold text-[var(--secondaryColor)]">
          My Bets
        </h1>
        <div className="flex items-center justify-between lg:justify-normal md:justify-normal sm:justify-normal">
          <select
            id="game-select"
          ...
          <button
            className="bg-[var(--logoutBg)] hover:bg-black text-white font-bold py-3 px-4 rounded"
            onClick={fetchMyBets}
          >
            Refresh
          </button>
                  ...
                  <>
                    {currentBets.map((bet, index) => (
                      <tr className="text-white border-b border-[var(--logoutBg)]">
                        ...
                        <td className="text-right py-4 px-6">{bet.gameCode}</td>
                        <td className="text-right py-4 px-6">{bet.platform}</td>
